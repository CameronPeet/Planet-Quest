...
{
	m_PerspectiveMatrix = glm::perspective(glm::radians(fov), aspectRatio, zNear, zFar);
	m_ProjectionMatrix = m_PerspectiveMatrix;
}

void Camera::SetOrthographic(float left, float right, float bottom, float top)
{
	m_OrthoMatrix = glm::ortho(left, static_cast<GLfloat>(right), bottom, static_cast<GLfloat>(top));
	m_ProjectionMatrix = m_OrthoMatrix;
}

void Camera::SetOrthographic()
{
	m_OrthoMatrix = glm::ortho(0.0f, static_cast<GLfloat>(1200), 0.0f, static_cast<GLfloat>(800));
	m_ProjectionMatrix = m_OrthoMatrix;
}
