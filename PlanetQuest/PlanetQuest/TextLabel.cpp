...
TextLabel::TextLabel(TextType _type, std::string text, std::string font)
{
	...
	program = 0;
	if (!CResources::GetShader("Text", program))
	{
	...
	if (CResources::GetFont("autodestructbb_reg.ttf", FontArray))
	{
		int i = 0;
		for (GLubyte c = 0; c < 128; c++)
		{
			Characters.insert(std::pair<GLchar, Character>(c, FontArray->characters[i]));
			i++;
		}
	}
	else
